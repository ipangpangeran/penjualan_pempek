...
const RekonRere = () => {
  ...
  const [items, setItems] = useState([
    { productId: '', productName: '', qty: 1, price: 0, subtotal: 0 },
  ]);
  ...
  const handleRowChange = (index, field, value) => {
    ...
    if (field === 'productId') {
      ...
      if (product) {
        ...
        // Priority: lookup from MBA_RERE_PRICES table, fallback to product.cogs
  ...
  const handleReset = () => {
    ...
    setNotes('');
    setItems([{ productId: '', productName: '', qty: 1, price: 0, subtotal: 0 }]);
  };
  ...
  // Format and copy to WA
  const handleCopyWA = (targetItems, targetDate, targetNotes, targetTotal) => {
    const valid = targetItems.filter((it) => it.productName && it.qty > 0);
    if (valid.length === 0) {
      showToast('Belum ada produk yang dipilih untuk disalin.', 'warning');
      return;
    }
    ...
    valid.forEach((d) => {
      text += `- ${d.productName} x${d.qty} pcs: ${formatRupiah(d.subtotal)} (${formatRupiah(d.price)}/pcs)\n`;
    });

    text += `\n----------------------------------\n`;
    text += `*Total Tagihan: ${formatRupiah(targetTotal)}*\n`;
    text += `----------------------------------\n`;
      ...
      <div className="bg-brand-card rounded-3xl p-6 border border-brand-border shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-brand-text-muted mt-1">
            Mencatat produk apa saja yang terjual beserta harga modal yang harus dibayarkan ke penjual asli (Mba Rere).
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    >
                      ...
                      <td className="p-3 text-right">
                        <input
                          ...
                          className="w-32 bg-brand-bg-input border border-brand-border text-brand-text focus:border-emerald-500 rounded-lg p-2 text-xs focus:outline-none text-right font-mono ml-auto block font-semibold"
                        />
                      </td>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between border-t border-brand-border pt-6 gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto justify-end">
              <button
                type="button"
                onClick={() => handleCopyWA(items, reconcileDate, notes, totalAmount)}
                ...
                title="Salin rincian belanja dalam format teks rapi untuk dikirim ke WA Mba Rere"
              >
